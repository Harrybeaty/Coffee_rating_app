...
# Check user_input does not = exit.
    while (user_input := input(MENU_PROMPT)) != "6":  # := is used to give a varibale a value within a statement.
        if user_input == "1":
            add_coffee(connection)

        elif user_input == "2":
            display_all_coffee(connection)
            ...
            find_top_10(connection)

        else:
            print("Invalid input, please try again")

def add_coffee(connection):
...
def display_all_coffee(connection):
